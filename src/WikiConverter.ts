...
class WikiConverter {
	...
	public process(): string {
		...
		// Extract sources
		const referenceSection = this.extractSection(
			this.unformattedWikiContent,
	...
	private extractSources(markdown: string): string[] {
	...
	private formatSourcesSection(sources: string[]): string {
		...
		const sourceSection: string = MarkdownHtmlConverter.convert(
			sources.join("\n")
		);
		...
		return `
			<section id="sources-section">
				<h1>Sources</h1>
				<p>In addition to our General List of Sources ([link](/sources.html)), we used these specific references: </p>
				<div class="container">
				${sourceSection}
				</div>
			</section>
